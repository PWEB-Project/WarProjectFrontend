...
export default {
    ...
    initialize() {
      ...
      submit () {
        api
        .addNewNews({"publicationDate": (new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10), "body": this.body, 
        "title": this.title, "countyId": this.getCountryByCode(this.valueCounty)[0].id, "writer": this.author, "newsType": {"id": 1}})
        .then(response => {
          console.log(response.data);
          this.dialog = false;
        })
